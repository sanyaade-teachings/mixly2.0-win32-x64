...
mixlyjs.saveserver = function (type, f) {
    $ = layui.jquery;
    var recordLine = {};
    recordLine.actionType = "saveserver";
    recordLine.time = new Date();
    window.actionArrayRecord.push(recordLine);
    var fileName = document.getElementById("boardSelector").value
    $.post("../../event.php", { "data": JSON.stringify(window.actionArrayRecord), "fileName": fileName, "type": type }, function (res) {
        if (res.code === 50) {
            layer.msg(res.msg);
            setTimeout("window.location.href='../../index.html'", 1000);
        } else {
