...
PyEngine.prototype.run = function(type) {
        ...
        $('body').append($(
            `<div id="skulpt-img" wrap="off" style="height:100%;display:none;">
                ...
                <div id="output_img" wrap="off" readonly
                    style="position: relative;height:100%;width:100%;background:#f9f9f9;outline:none;overflow:auto"></div>
    ...
    /* layui.use('layer',function () {
      layer=layui.layer;
      $=layui.jquery;
      $.post("../../file.php",{"data":data,"fn":fn,"code":"0","type":type},function (res) {
          if(res.code===200){
              layer.msg(res.msg);
          }else if(code===100){
              layer.msg(res.msg);
    ...
    //设置文本输出
        output: function(lineText){
            try {
                lineText = lineText.replace(/\\(u[0-9a-fA-F]{4})/g, function (s) {
