...
class WikiPage {
    ...
    getPagePath(contentPath, contentList) {
        ...
        if (contentPath.length === 1) {
            for (let key in contentList) {
                ...
                if (typeof child?.link === 'object'
                 && child.link.title === contentPath[0]) {
    ...
    updateContentFile() {
        const wikiContentPath = path.resolve(Env.indexPath, './wiki/content.md');
        const defaultWikiPath = path.resolve(Env.indexPath, './wiki/wiki-libs/' + Code.LANG);
        const wikiHomePagePath = path.resolve(defaultWikiPath, './home');
        const thirdPartyLibsPath = path.resolve(Env.indexPath, './libraries/ThirdParty/');
    ...
    updateWiki() {
        const args = [
            {
                default: path.resolve(Env.indexPath, './wiki/wiki-libs/'),
                thirdParty: path.resolve(Env.indexPath, './libraries/ThirdParty/'),
                content: path.resolve(Env.indexPath, './wiki/content.md')
            }
        ];
        if (this.gotoInfo) {
...
WikiManager.openWiki = (gotoInfo) => {
    ...
    if (!WikiManager.wiki || WikiManager.wiki.isDestroyed) {
        const wikiPath = path.resolve(Env.indexPath, '../common/wiki/index.html');
