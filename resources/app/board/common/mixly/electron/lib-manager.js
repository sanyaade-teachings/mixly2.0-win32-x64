...
LibManager.loadLibsAndUpdateJsCssList = (doFunc = () => {}) => {
    const {
        ...
        css,
        xml
    } = LibManager.getLibs(path.resolve(Env.indexPath, './libraries/ThirdParty/'));
...
LibManager.reloadThirdPartyLibs = () => {
    ...
    LibManager.convertLibs(path.resolve(Env.indexPath, './libraries/ThirdParty/'));
    ...
    layero.find('button').off().click(function () {
        ...
        switch (mId) {
            ...
            case 'del':
                ...
                break;
            case 'open-folder':
                const arduinoLibPath = path.resolve(Env.indexPath, 'libraries/myLib');
                const pyLibPath = path.resolve(Env.indexPath, 'build/lib');
                let needOpenedPath;
                switch (delType) {
                    case 'ARDUINO':
...
LibManager.compareLibConfig = (cloudConfig) => {
    ...
    const libDir = path.resolve(Env.indexPath, 'libraries/ThirdParty', path.basename(url, '.zip'));
...
LibManager.onclickImportLibs = () => {
    ...
    if (Nav.codeEnv === 'c') {
        codeLibPath = path.resolve(Env.indexPath, './libraries/myLib');
        codeLibUrl = LibManager.URL.arduino;
    } else {
        codeLibPath = path.resolve(Env.indexPath, './build/lib');
...
LibManager.onclickManageLibs = () => {
    ...
    const thirdPartyPath = path.resolve(Env.indexPath, 'libraries/ThirdParty');
    const arduinoLibPath = path.resolve(Env.indexPath, 'libraries/myLib');
    const pythonLibPath = path.resolve(Env.indexPath, 'build/lib');
    ...
    switch (extname) {
        case '.xml':
            ...
            if (fs_extend.isdir(libsPath)) {
                ...
                const myLibPath = path.resolve(Env.indexPath, 'libraries/myLib');
        ...
        case '.mil':
            ...
            break;
        case '.py':
            const pyPath = path.resolve(Env.indexPath, 'build/lib', path.basename(filePath));
