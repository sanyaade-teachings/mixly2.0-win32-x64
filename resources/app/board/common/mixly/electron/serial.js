...
Serial.connect = function (port = null, baud = null, endFunc = (code) => {}) {
    ...
    const { dom } = portObj;
    if (portObj.serialport && (portObj.serialport.isOpen || portObj.serialport.opening)) {
        if (portObj.serialport.path === port) {
            return;
        } else {
            portObj.serialport.close();
            return;
        }
    }
    portObj.serialport = new SerialPort({
        path: port,
        baudRate: baud - 0,  //波特率
        dataBits: 8,    //数据位
        parity: 'none',  //奇偶校验
    ...
    let receiveBuff = [], chineseStr = "", buffLength = 0;
    const parser = new ReadLine();
    const parserBytes = portObj.serialport.pipe(new ByteLength({ length: 1 }))
    ...
    parserBytes.on('data', function (data) {
        ...
        if (portObj.toolConfig && !portObj.toolConfig.receiveStr) {
            if (data[0] === 10) {
                ...
                let numStr = data[0].toString(16).toUpperCase();
                if (numStr.length === 1)
                    numStr = '0' + numStr;
...
Serial.writeByteArr = function (port, sendDataStr, sendDataTail) {
    ...
    nowSerialport.write(sendDataNum);
    ...
    if (sendDataTail) {
        nowSerialport.write(sendDataTail);
    }
}
...
Serial.writeString = function (port, sendDataStr, sendDataTail) {
    ...
    if (nowSerialport) {
        ...
        if (sendDataStr) {
            nowSerialport.write(sendDataStr + sendDataTail);
        }
    }
}
...
Serial.setBaudRate = (port, baud) => {
    ...
    if (newSerialport && newSerialport.isOpen) {
        newSerialport.update({ baudRate: baud - 0 }, err => { });
    }
}
